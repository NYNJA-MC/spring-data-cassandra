    ...
    stage('Build') {
      ...
      steps {
        container('mvn') {
			...
			sh '''echo > settings.xml '<?xml version="1.0" encoding="UTF-8"?> <settings xmlns="http://maven.apache.org/SETTINGS/1.0.0" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="http://maven.apache.org/SETTINGS/1.0.0 http://maven.apache.org/xsd/settings-1.0.0.xsd"> <servers>  <server> <id>nynjagroup.jfrog.io</id> <username>'${USER}'</username> <password>'${PASS}'</password> </server></servers> </settings>'
'''
			sh '''
                        mvn --version
                        '''
			...
			sh 'mvn --settings settings.xml -DskipTests=true -DaltDeploymentRepository=nynjagroup.jfrog.io::default::https://nynjagroup.jfrog.io/nynjagroup/libs-snapshot-local deploy'
		    }
